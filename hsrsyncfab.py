...
def fablink_tmpfiles(host, cluster, next_file, tmp_dir):
	fabcmd = "cp -l " + next_file + " " + tmp_dir
	with settings(host_string=host):
		if cluster == 'localhost':    	
			local(fabcmd)
		else:
...
def request_parser(request_begin_utc, request_end_utc, request_start, request_stop, copydir, src_mchn, cluster):
	...
	if infoParseLast and infoParseCurrent:
		...
		if BUFFSTART < ALERTSTOP:
			...
			logging.info( "SN_STOP part2 file " + str(sn_stop_file_str2))			  
...
#			logging.info( "created subdir for relevant hs files")
			fabmkdir_tmpdir(src_mchn, cluster, tmp_dir)
...
if __name__=="__main__":
	...
	def usage():
		...
		usage :: hsrsyncfab.py [options]
			...
			hsrsyncfab reads UTC timestamps or DAQ timestamps, calculates the requested hitspool file indexes and ships the data to copydir.
			"""
		sys.exit(1)


	##take arguments from command line and check for correct input
	opts, args = getopt.getopt(sys.argv[1:], 'hb:e:s:d:', ['help','begin','end', 'source', 'destination'])
	for o, a in opts:
		if o == '-b':				
			if not a.isdigit():
...
#				request_end_utc = datetime.strptime(str(a), "%Y-%m-%d %H:%M:%S.%f")
	...
	if cluster == "localhost":
		logfile = "/home/david/TESTCLUSTER/testhub/logs/hsrsyncfab.log" 
	else:
		logfile = "/home/pdaq/HsInterface/logs/hsrsyncfab.log"
	...
	request_parser(request_begin_utc, request_end_utc, request_start, request_stop, copydir, src_mchn, cluster)
